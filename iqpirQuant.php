...
function describeParams() {
	echo "The sample map file contains 3 required tab-delimited columns listed below\n";
...
function writeParams($print = false) {
...
function createParamFiles($filepaths, $email) {
$files = array();
$dir = getcwd() . "/";
...
if(count($argv) < 2) {
	...
	echo "         LOGRATIOS APPORTS(=x,y,...)                 [after RUN: computes crosslink channel apportionments for all or specified samples]\n";
	...
	echo "\n";
	echo "  or:    \033[1m{$self} PRINTPARAMS \033[0m [to print sample file contents to terminal ]\n";
	echo "         \033[1m{$self} WRITEPARAMS \033[0m [to write sample iqpir.params file in current directory ]\n";
	echo "         \033[1m{$self} WRITEPARAMS < /full/path/to/sample1/pepxml,/full/path/to/sample2/pepxml,...> < email >  \033[0m [to write sample iqpir.params file in current directory]\n";
	echo "         \033[1m{$self} DESCRIBE \033[0m [desribes how sample.map and sample.def are formatted for analysis ]\n";
	echo "\n";
	echo "\n  Will read the XLinkProphet file name listed in the parameter file, then write that file containing data filtered from it to achieve the specified FDR\n";
	echo "\n";
...
if(count($argv)===2) {
	...
	createParamFiles(split(",", $argv[2]), $argv[3]);
	exit(1);
}
else if(count($argv) == 5 && $argv[1] == "WRITEPARAMS" && $argv[4] == "MULTI_CHANNEL") {
	createParamFiles(split(",", $argv[2]), $argv[3], true);
	exit(1);
}

if(count($argv)===3 && $argv[2]==="COMBO") {
	$self = "/data/users/keller/software/" . $self;
...
if(count($argv)===3) {
	...
	if($argv[2]==="LOGRATIOS") {
		...
		if($nextpos===0) {
			...
			$samples = split(",", substr($argv[3], strlen("APPORTS=")));
	...
	if($argv[2] == "LOGRATIOS" && $argv[3] == "INTRA") {
